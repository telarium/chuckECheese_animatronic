...
class AutomatedPuppeteering:
	...
	def monitor_audio(self, file_path):
		...
		try:
			# If it's an OGG file, convert to WAV using PyDub
			if file_path.endswith('.ogg'):
				audio = AudioSegment.from_ogg(file_path)
				# Convert the OGG file to WAV format in-memory
				wav_io = io.BytesIO()
				audio.export(wav_io, format="wav")
				wav_io.seek(0)
				sample_rate, data = wavfile.read(wav_io)  # Read as WAV
			...
			# Play the audio file using pygame.mixer (use music for better control)
